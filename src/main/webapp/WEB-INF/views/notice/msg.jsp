    ...
    <UL>
      <c:choose>
        <c:when test="${code == 'create_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">새로운 공지사항 [${noticeVO.noticetitle }] 등록했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${code == 'create_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">새로운 공지사항 [${noticeVO.noticetitle }] 등록에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${code == 'update'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">공지사항 수정에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${code == 'delete_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">[${noticeVO.noticetitle }] 공지사항 삭제에 성공했습니다.</span>
          </LI>                                                                      
        </c:when>        
